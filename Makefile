COMPILE.d = $(DC) $(DFLAGS) -d -O2 $(CPPFLAGS) $(TARGET_ARCH) -c -of=$@
LIBS=-defaultlib=stdc++
DEBUG=0
COMFLAGS=
DLINK=$(COMFLAGS)
...
tar:
	git archive master --prefix=cheesecutter-2.5.0/ | bzip2 > cheesecutter-2.5.0-d2-macosx-src.tar.bz2
# --------------------------------------------------------------------------------
